...
import ge.c0d3in3.healthify.extensions.go
import ge.c0d3in3.healthify.presentation.auth.repository.AuthRepository
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class SignInViewModel(app: Application, private val authRepository: AuthRepository) :
    BaseViewModel(app) {
    ...
    private fun firebaseSignIn(email: String, password: String) {
        viewModelScope.launch(Dispatchers.IO) {
            authRepository.signIn(email, password).go(
                onSuccess = {
                    if (it.user == null)
    ...
    private suspend fun saveUser(firebaseUser: FirebaseUser) {
        authRepository.getUser(firebaseUser.uid).go(
            onSuccess = {
